...
We have organized these rules in three groups. %sara: i think here is a good place to add the diagram (but we need to update for the three groups and not four as it is in the figure now): https://docs.google.com/drawings/d/1iS1pLc9OldLMe_jhNT6OGIbT9s5KvlHpUoDd19vHkCY/edit?usp=sharing
%sara: suggestion for figure alt text: Diagram of how the 10 rules are organized into four groups: grounding rules (rules 1, 2, and 10), people-related rules (rules 3 and 4), design rules (rules 5, 6, and 7), and make-it-happen rules (rules 8 and 9). The diagram has five rows. Each line contains on the left the title of the group in uppercase and on the center rectangular boxes with a short title for each rule. Each rule box is colored based on their group. First row is for the grounding rules 1. Multidimensional diversity and 2. Safe and inclusive environment with grey boxes. Second row is for people-related rules 3. Organizing team and 4. Unbias with green boxes. Third row is for the design rules 5. Online component, 6. Accessibility for disabilities, 7. Language inclusivity. The fourth row is for the make-it-happen rules 8. Communication and 9. Financial support and budget. The last row is for the grounding rule 10. Diversity and inclusion as a process.    
